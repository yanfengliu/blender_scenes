# AGENTS.md — blender_scenes

## What this is

Procedural Blender scenes built from editable Python source through the official Blender Lab MCP. The current scene is a strawberry-glazed donut with three inspection cameras. Toolchain: Blender 5.2, Python, PowerShell, and uv.

Scene source lives in `scenes/`; setup, MCP lifecycle, and build/render tools live in `tools/`. See `README.md` for setup and rebuild instructions. Repository retention rules live in [docs/policies/local-rules.md](docs/policies/local-rules.md).
...
- Fleet membership and instruction sync: run `npm test` and `npm run sync-canon` from `../fleet`.
- Python syntax, without importing Blender or starting services: `.venv\Scripts\python.exe -c "import ast, pathlib; [ast.parse(p.read_text(encoding='utf-8'), filename=str(p)) for d in ('scenes', 'tools') for p in pathlib.Path(d).glob('*.py')]"` from this repository after setup.
- For scene or MCP workflow changes, run `.venv\Scripts\python.exe tools\build_donut_mcp.py` and inspect `outputs/donut/hero.png`, `overhead.png`, and `detail.png`. The runner checks the MCP results and saved scene; Python syntax alone does not verify geometry or rendered quality. There is no standalone automated scene test suite.

## Invariants & boundaries

- `scenes/donut.py` is the procedural scene source; generated scenes and renders live under ignored `outputs/` according to the linked retention rules.
- Use the official Blender Lab MCP implementation pinned by `tools/setup_blender_mcp.ps1`. The similarly named community `uvx blender-mcp` package is a different implementation. The MCP SDK pin in setup preserves the API the official package imports.
- `tools/build_donut_mcp.py` owns its headless Blender instance and MCP connection. It does not attach to an open Blender session or change user preferences; interactive editing is a separate workflow documented in `README.md`.

## Conventions

Keep reusable scene logic in `scenes/` and setup/process management in `tools/`. Record behavior changes in `docs/devlog/summary.md`.
